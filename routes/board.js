...
router.get("/list", function(req, res, next){
    ...
    var rowPerPage=5; //페이지당 보여줄 글 목록:10개
    ...
    con.query(sql, function(error, record){
        if(error){
            ...
            lastPage = Math.floor(totalRow/rowPerPage);
    ...
    var sql="select * from board order by board_id desc limit ?,?";
...
router.get("/detail", function(req, res, next){
    if(req.session.displayName){
        ...
        var sql = "select * from board where board_id="+board_id+";";
        ...
        con.query(sql, function(error, record, fields){
            ...
            if(error){
                ...
                var test = record[3];
                console.log(test[1]);
                res.render("G_boardDetail", {"board":record[0][0], 
